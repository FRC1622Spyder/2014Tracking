...
void Vision::filterContours()
{
	...
	{
		while((this->contours[j].size() <= 4 )&&(j+1 < this->contours.size())) { //if chain contains less than 4 points				
	...
	{
		while(((5000.0f<this->boundRect[j].area())||(2000.0f>this->boundRect[j].area()))&&(j+1 < this->contours.size())) { //a sort of band pass filter for rectangle area.			
			this->contours.erase(this->contours.begin()+j);
			this->hierarchy.erase(this->hierarchy.begin()+j);
